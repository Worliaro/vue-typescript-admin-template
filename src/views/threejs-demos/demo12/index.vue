...
export default class extends Vue {
  ...
  makeTextGeometry(
    ...
    const textGeometry = new THREE.TextGeometry(text, fontParameters)
  ...
  init() {
    ...
    // 创建相机
    const defaultPosition = new THREE.Vector3(0, 200, 300)
    ...
    // 灯光
    scene.add(this.makeAmbientLight(0xd1d1d1))
    scene.add(
      this.makeSpotLight(0xeedfcc, new THREE.Vector3(-100, 200, 100), true)
    )
    // 文字网格模型
    ...
    scene.add(textMesh)
    // 格网辅助
    const gridHelper = this.initGridHelper(1200, 60, 0xcd3700, 0x424242)
