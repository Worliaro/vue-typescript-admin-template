...
export default class extends Vue {
  ...
  initContent(scene: THREE.Scene) {
    ...
    cube.position.x = -55
    const cubeEdgesGeometry = new THREE.EdgesGeometry(cubeGeometry, 1)
    const cubeLine = new THREE.LineSegments(cubeEdgesGeometry, edgesMtl)
    // 棱柱
    const cylinderGeometry = new THREE.CylinderGeometry(25, 25, 50, 10)
    const cylinder = new THREE.Mesh(cylinderGeometry, material)
    cylinder.position.x = 55
    const cylinderEdgesGeometry = new THREE.EdgesGeometry(cylinderGeometry, 1)
    const cylinderLine = new THREE.LineSegments(cylinderEdgesGeometry, edgesMtl)
    // 棱锥
    const coneGeometry = new THREE.ConeGeometry(25, 50)
    const cone = new THREE.Mesh(coneGeometry, material)
    const coneEdgesGeometry = new THREE.EdgesGeometry(coneGeometry, 1)
    const coneLine = new THREE.LineSegments(coneEdgesGeometry, edgesMtl)
  ...
  init() {
    ...
    vm.initLight(scene)
    const { cube, cylinder, cone } = vm.initContent(scene)
