...
export default class extends Vue {
  ...
  initContent(scene: THREE.Scene) {
    const circleGeometry = new THREE.CircleGeometry(50, 40, 3, 2 * Math.PI)
    circleGeometry.name = 'circleGeometry'
    const circle = this.createMesh(circleGeometry)
    scene.add(circle)
    return {
      circle,
    }
  }
  createMesh(geometry: THREE.BufferGeometry) {
    const meshMaterial = new THREE.MeshNormalMaterial()
    meshMaterial.side = THREE.DoubleSide
    const wireFrameMaterial = new THREE.MeshBasicMaterial()
    wireFrameMaterial.wireframe = true
    const circle = new SceneUtils.createMultiMaterialObject(geometry, [
      meshMaterial,
      wireFrameMaterial,
    ])
  ...
  initGUI(scene: THREE.Scene, createPlaneMesh: Function) {
    class GuiControls {
      ...
      private thetaStart: any
      private thetaLength: any
      constructor(scene: THREE.Scene) {
        const circle = scene.getObjectByName('circleMesh') as THREE.Group
        if (circle) {
          const circleGeometry = circle.children[0].geometry
          if (circleGeometry) {
    ...
    const gui = new GUI()
    gui.add(guiControls, 'radius', 0, 200).onChange((e) => {
      const circle = scene.getObjectByName('circleMesh') as THREE.Group
      if (circle) {
        const circleGeometry = circle.children[0].geometry
        if (circleGeometry) {
          ...
          scene.remove(circle)
          const newCircle = createPlaneMesh(
            new THREE.CircleGeometry(e, segments, thetaStart, thetaLength)
          )
          scene.add(newCircle)
        }
      }
    })
    gui.add(guiControls, 'segments', 3, 90).onChange((e) => {
      const circle = scene.getObjectByName('circleMesh') as THREE.Group
      if (circle) {
        const circleGeometry = circle.children[0].geometry
        if (circleGeometry) {
          ...
          scene.remove(circle)
          const newCircle = createPlaneMesh(
            new THREE.CircleGeometry(radius, e, thetaStart, thetaLength)
          )
          scene.add(newCircle)
        }
      }
    })
    gui.add(guiControls, 'thetaStart', 0, 12).onChange((e) => {
      const circle = scene.getObjectByName('circleMesh') as THREE.Group
      if (circle) {
        const circleGeometry = circle.children[0].geometry
        if (circleGeometry) {
          ...
          scene.remove(circle)
          const newCircle = createPlaneMesh(
            new THREE.CircleGeometry(radius, segments, e, thetaLength)
          )
          scene.add(newCircle)
        }
      }
    ...
    gui.add(guiControls, 'thetaLength', 0, Math.PI * 2).onChange((e) => {
      ...
      if (circle) {
        ...
        if (circleGeometry) {
          ...
          scene.remove(circle)
          const newCircle = createPlaneMesh(
            new THREE.CircleGeometry(radius, segments, thetaStart, e)
          )
          scene.add(newCircle)
        }
      }
  ...
  init() {
    ...
    // vm.initLight()
    const { circle } = vm.initContent(scene)
